...
def view_log():
    try:
        with open("restaurant_log.txt", "r") as file:
            print("\nTimestamp | Restaurant | Cuisine | Location | Rating | Notes")
            print("-" * 80)
            for line in file:
                print(line.strip())
    except FileNotFoundError:
        print("No restaurant log found.")

def filter_high_rated():
    try:
        with open("restaurant_log.txt", "r") as file:
            print("\nTop-Rated Restaurants (8–10):")
            print("-" * 80)
            for line in file:
...
def show_count():
    try:
        with open("restaurant_log.txt", "r") as file:
            ...
            print(f"\nTotal restaurants logged: {len(lines)}")
    except FileNotFoundError:
        print("No restaurant log found.")
...
def main():
    while True:
        ...
        if choice == "1":
            ...
            if confirm.lower() == "y":
                with open("restaurant_log.txt", "w") as file:
                    file.write("")
                print("Restaurant log cleared.")
            else:
                print("Cancelled. Restaurant log not cleared.")
